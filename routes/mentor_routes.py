...
@check_roles(["MENTOR"])
def update_mentor(profile: MentorProfileSchema, current_user: dict = Depends(verify_token),
...
@mentor_routes.get("/job/{job_id}")
@check_roles(["MENTOR"])
def get_job_by_id(job_id: int, current_user: dict = Depends(verify_token), db: Session = Depends(get_db)):
    """
    Retrieve a specific job by its ID.
    """
    # Call the repository function to fetch the job
    job = view_job_by_id(job_id, db)

    # If the job is not found, raise HTTP 404 error
    if not job:
...
@check_roles(["MENTOR"])
def apply_for_job(application: ApplicationMentorSchema, current_user: dict = Depends(verify_token),
                  db: Session = Depends(get_db)):
    user_id = get_userid_by_email(db, current_user['user'])
...
@check_roles(["MENTOR"])
def show_interested_intern_for_project(project_id, current_user: dict = Depends(verify_token),
                                       db: Session = Depends(get_db)):
    user_id = get_userid_by_email(db, current_user['user'])
...
@check_roles(["MENTOR"])
def review_applications(payload: InternModifyApplications, current_user: dict = Depends(verify_token),
                        db: Session = Depends(get_db)):
    user_id = get_userid_by_email(db, current_user['user'])
...
@check_roles(["MENTOR"])
def show_project_intern_for_project(project_id, current_user: dict = Depends(verify_token),
                                    db: Session = Depends(get_db)):
    user_id = get_userid_by_email(db, current_user['user'])
...
@check_roles(["MENTOR"])
def show_project_intern_for_project(task: TaskSchema, current_user: dict = Depends(verify_token),
                                    db: Session = Depends(get_db)):
    user_id = get_userid_by_email(db, current_user['user'])
...
@mentor_routes.get("/view_tasks_by_job/{job_id}")
@check_roles(["MENTOR"])
def view_all_tasks(job_id: int, current_user: dict = Depends(verify_token), db: Session = Depends(get_db)):
    """
    Retrieve all tasks assigned to the current mentor and job.
    """
    # Retrieve user_id using the current user's email
    user_id = get_userid_by_email(db, current_user['user'])

    # Call the repository function to retrieve tasks for the current mentor and job
    tasks = retrieve_tasks_by_job(user_id, job_id, db)
...
@check_roles(["MENTOR"])
def update_task(task_id: int, task_payload: TaskSchema, current_user: dict = Depends(verify_token),
                db: Session = Depends(get_db)):
